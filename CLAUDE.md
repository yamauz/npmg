...
- **A11y / Best Practices は 100 未満で落ちる**
- **TBT・CLS も閾値あり**(200ms / 0.1)
- **Performance と LCP は閾値を置いていない。** 計測ごとに 10 点以上揺れる(実測 78〜98)ため。Cloudflare のコールドスタートが 400ms 以上あり、これが LCP の半分近くを占める。数字はコメントに出るので人が見る
- **SEO はプレビューでは必ず 66 になる**(Cloudflare が `x-robots-tag: noindex` を付けるため)。原理的に検査できないので CI からは外してある。本番では 100

**Performance を測るときは 1 回の数字で判断しない。** ウォームアップしてから 3〜5 回測る。

既知の劣化: **CLS が 0.014 → 0.024〜0.082 に悪化している**(2026-08-31、Inter を外した副作用)。Lighthouse の指す発生源はヘッダーの `.container` で、原因は Web フォント読み込み。「good」の範囲(0.1 未満)には収まっているので許容しているが、詰めるならフォントのフォールバック指標合わせ(`size-adjust` 等)が次の一手。Mermaid の遅延読み込みは無関係(`min-height` も `#fallback` の枠も効果がないことを実測で確認済み)。
