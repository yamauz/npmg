...
1. **二重支持の原則**: 抽象概念(hoisting、phantom dependency、CAS、シンボリックリンク解決など)は、必ず【Mermaid 図/TermBlocks(実測ターミナル)/具体的な比喩/図版プレースホルダー】のうち **2 つ以上**で支える。文章だけで説明を終えない
...
### TermBlocks の記法ルール(グローバル登録済みコンポーネント)
```html
<TermBlocks
  title="zsh — 用途"
  :lines="[
...
/>
```
- タグ内に空行禁止/文字列はシングルクォート/出力は実測値のみ
- **同じ内容の静的コードブロックを併置しない**(コードブロック自体がターミナル表示になるため、二重に見える)
- `pause` は受け取るが無視される(再生機能は廃止済み)
