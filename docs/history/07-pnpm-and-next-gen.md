...
pnpm は 2016 年、Zoltan Kochan 氏が開発を開始しました(v1 のリリースは 2017 年)。奇しくも yarn と同じ年に生まれていますが、ここで疑問が湧きます。yarn が「遅い・非決定的・オフラインに弱い」を解決したのなら、なぜもう 1 つ別のツールが必要だったのでしょうか。答えは、yarn が手を付けなかった問題にあります。yarn が「npm を速く、確実にする」ことを目指したのに対し、pnpm は当時の npm(そして yarn v1)が共有していた **2 つの構造的な問題**に切り込みました。
