...
本書は **npmg**(**N**ode.js **P**ackage **M**anager **G**uide)——書名『node_modulesの深層』として公開している、Node.js のパッケージマネージャーを仕組みから理解するためのオンライン教科書です。
...
- pnpm のストア構造とリンク戦略を理解し、そのアドバンテージを列挙できる
- phantom dependency やビルドスクリプト起因の典型的なトラブルに、正体を理解した上で対処できる

## 構成

全 12 章 + 付録の 4 部構成です。前から順に読むことを想定していますが、pnpm を今すぐ触りたい方は Part III から読み、必要に応じて前の章に戻る読み方もできます。
...
多くの章に「実験」セクションがあります。ターミナルでコマンドを実行し、実際の出力を見ながら仕組みを確認する構成です。Node.js 20 以降がインストールされていれば、追加の準備はほとんど必要ありません。実験用の使い捨てディレクトリ(例: `~/sandbox/pm-play`)を 1 つ用意しておくと便利です。
