...
::: warning つまずきポイント — `du` の数字が同じでも、ディスクは共有されている
...
`.pnpm` の中は、一見すると呪文のようなフォルダ名が並びますが、規則は単純です。規則を一般論で覚える前に、公式ドキュメントが使う**最小の例**を見るのが早道です。「foo@1.0.0 が bar@1.0.0 に 1 つだけ依存している」——それだけのプロジェクトを pnpm でインストールすると、node_modules はこうなります(`<store>` はグローバルストアからのハードリンク/クローンを表します)。
...
順に確認します。foo のコードの実体は `.pnpm/foo@1.0.0/node_modules/foo` の中にあります(ルートの `node_modules/foo` は案内板にすぎません)。Node.js はこの**実体の場所**から、[3章](/basics/03-node-modules)で学んだルールそのままに探索を始めます。自分の直下に `node_modules` はない。では 1 つ上——そこは `.pnpm/foo@1.0.0/node_modules` で、**bar へのシンボリックリンクがちょうど置いてある**場所です。たった 1 階層で発見。リンクの先の実体が読み込まれて、解決完了です。
