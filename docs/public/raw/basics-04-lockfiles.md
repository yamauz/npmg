...
「控えのとおりに買う。控えと注文書が食い違っていたら、勝手に判断せず報告する」という動きです。この 3 つを実際に見てみましょう。[3章](https://npmg.yamauz.workers.dev/basics/03-node-modules)の flat-lab(express インストール済み)で `npm ci` を実行したあと、`npm pkg set` で package.json の要求だけを `^4.0.0` に書き換え、わざと矛盾を作ります。
