...
[2章](https://npmg.yamauz.workers.dev/basics/02-package-json-and-semver)で「同じ package.json なのに人によって違うバージョンが入る」という伏線を張り、[3章](https://npmg.yamauz.workers.dev/basics/03-node-modules)では「同じ入力から違う形のツリーができる」非決定性を見ました。Part I の締めくくりとなるこの章では、この 2 つの問題への答え——ロックファイル(lockfile)——の中身と役割を理解します。
...
**[注意] つまずきポイント: これは「ロックファイルがない場合」の話**
...
[図 4-1 lockfile がないチームで起きるバージョンのズレ]
...
package.json とロックファイルの関係は、**品書きと仕入れ伝票**にたとえられます。

[2章](https://npmg.yamauz.workers.dev/basics/02-package-json-and-semver)で見たとおり、らぁめん濃度盛汁流の品書きには「豚バラ 3 枚から 5 枚」と幅で書いてあります。要求を範囲で書いた注文書です。一方ロックファイルは、実際に仕入れをした人が残した**伝票**です。「◯◯精肉店の豚バラ、ロット XX を 4 枚」——次に仕入れに行く人がこの伝票のとおりに買えば、**誰が行っても寸分違わず同じ一杯**になります。

品書きだけを渡された新入りが、今日はたまたま 5 枚で作ってしまう。これが章の冒頭で見た「新メンバーの環境でだけテストが落ちる」の正体です。品書きは幅を語り、伝票は結果を語る。**幅で共有するから味がぶれるのであって、伝票まで共有すればぶれない**——これがロックファイルの発想のすべてです。
...
重要なのは 3 行目です。ロックファイルは、あなたが宣言した数十個だけでなく、[1章](https://npmg.yamauz.workers.dev/basics/01-what-is-a-package-manager)で見た「依存の依存」まで含めた**全パッケージ**の確定値を記録します。3 章の非決定性——どれを hoist してどれをネストしたかというツリーの形——も npm のロックファイルには記録されるため、「解決」の工程を丸ごとスキップして同じ node_modules を再現できるのです。

## 1 エントリを読み解く

では、その「伝票」には具体的に何が書かれているのでしょうか。中身は決して怖いものではありません。1 章の実験で left-pad を入れたときに生成された package-lock.json から、left-pad のエントリを見てみます。
...
ロックファイルは**必ずリポジトリにコミットしてください**。自動生成されるファイルは `.gitignore` に入れたくなるものですが、ロックファイルは例外です。コミットして初めて「チーム全員と CI が同じ伝票で仕入れをする」状態になり、図 4-1 のズレが消えます。

**[注意] つまずきポイント: コミットするのは lockfile、しないのは node_modules**
...
[図 4-2 lockfile を介した再現可能なインストールフロー]
...
「伝票のとおりに仕入れる。伝票と品書きが食い違っていたら、勝手に判断せず報告する」という動きです。この 3 つを実際に見てみましょう。[3章](https://npmg.yamauz.workers.dev/basics/03-node-modules)の flat-lab(express インストール済み)で `npm ci` を実行したあと、`npm pkg set` で package.json の要求だけを `^4.0.0` に書き換え、わざと矛盾を作ります。

```sh
$ npm ci
added 68 packages, and audited 69 packages in 2s
$ npm pkg set dependencies.express=^4.0.0
$ npm ci
npm error code EUSAGE
npm error `npm ci` can only install packages when your package.json and package-lock.json or npm-shrinkwrap.json are in sync. Please update your lock file with `npm install` before continuing.
npm error Invalid: lock file's express@5.2.1 does not satisfy express@^4.0.0
...
**[注意] つまずきポイント: 「lockfile があれば `npm install` でも同じ」ではない**
...
[3章](https://npmg.yamauz.workers.dev/basics/03-node-modules)の実験で express を入れたディレクトリに、package-lock.json ができているはずです。まず全体の規模を見てみます。
...
- ロックファイルは推移的依存まで含む全パッケージの確定バージョン・取得 URL・integrity を記録した「仕入れ伝票」
- 必ずコミットする。チームと CI が同じ伝票を共有して初めて再現性が生まれる
