...
[1章](https://npmg.yamauz.workers.dev/basics/01-what-is-a-package-manager)で名前だけ出した、らぁめん濃度盛汁流。この店には代々の大将がいます。npm はその**初代**です。

初代は、何もなかったところに店を構えました。それまで客は具材を自分で market に買いに行き、自分で刻んでいた——[1章](https://npmg.yamauz.workers.dev/basics/01-what-is-a-package-manager)の「パッケージマネージャーがない世界」です。初代は「頼めば全部こっちで揃える」と言い切り、それだけで行列ができました。

やり方は豪快でした。若い頃は具材を頼まれた順に几帳面に重ねていた(ネスト)。しかし丼が深くなりすぎて底が見えない。そこで思い切って**全部を丼の表面に広げた**のが v3 の全部乗せ(hoisting)です。客の受けはよかった。ただし [3章](https://npmg.yamauz.workers.dev/basics/03-node-modules)で見たとおり、頼んでいない具材まで乗っているせいで、客が勝手にそれをあてにしはじめる。初代はこの癖を最後まで直しませんでした。
