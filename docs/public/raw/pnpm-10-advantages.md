...
[9章](https://npmg.yamauz.workers.dev/pnpm/09-how-pnpm-works)で、pnpm のストアとリンクの仕組みを理解しました。この章では、その仕組みが実務に何をもたらすのかを 7 つのアドバンテージに整理します。pnpm の採用を検討・評価する場面で、そのまま判断材料として使える章です。
...
npm は幽霊依存を黙って通し、pnpm は入口で止める。npm 側でこうなる理由は[3章](https://npmg.yamauz.workers.dev/basics/03-node-modules)の hoisting、pnpm 側でこうなる理由は[9章](https://npmg.yamauz.workers.dev/pnpm/09-how-pnpm-works)のシンボリックリンク構造で説明したとおりです。
...
pnpm はワークスペース機能を後付けではなく中核機能として磨いてきました。ルートの `pnpm-workspace.yaml` にパッケージの場所を宣言し、`workspace:` プロトコルで「必ずローカルのパッケージを使う(レジストリへ勝手にフォールバックしない)」参照を張れます。v9.5.0 で導入された **catalogs** はワークスペース全体の依存バージョンを一元管理する仕組みで、`--filter` による対象指定と合わせて、モノレポ運用の道具が標準装備されています。詳しくは[11章](https://npmg.yamauz.workers.dev/pnpm/11-workspaces)で 1 章かけて解説します。
