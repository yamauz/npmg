...
[1章](https://npmg.yamauz.workers.dev/basics/01-what-is-a-package-manager)の実験で、`npm install left-pad` が package.json に `"left-pad": "^1.3.0"` と書き込むのを見ました。なぜ `1.3.0` ちょうどではなく `^` 付きなのでしょうか。そもそも、この 1 行はどこに書かれるべきなのでしょうか。
...
なお、この約束はあくまで公開者の自己申告です。MINOR アップデートのつもりが実際にはバグを含んでいた、という事故は現実に起こります。この「約束は破られうる」という点が、[4章](https://npmg.yamauz.workers.dev/basics/04-lockfiles)で見るロックファイルの存在理由につながります。
...
[1章](https://npmg.yamauz.workers.dev/basics/01-what-is-a-package-manager)のらぁめん濃度盛汁流でいえば、package.json は厨房に貼られた**品書き**です。そこに書いてあるのは「豚バラ 3 枚」という一点ではなく、「豚バラ 3 枚から 5 枚まで」という**幅**です。`^` は「同じ豚バラである限り、多少の枚数の増減は任せる」、`~` は「増減は 1 枚まで」、記号なしは「きっかり 3 枚」。幅を持たせておけば仕入れの都合に合わせられますが、そのぶん**今日の一杯が何枚だったかは品書きを見てもわからない**。この「わからなさ」が、4 章で仕入れ伝票(ロックファイル)が必要になる理由に直結します。
...
**[注意] つまずきポイント: アプリでは区別しなくても「動いてしまう」**
...
**[補足] どんなときに peerDependencies を書くのか**
...
**[注意] つまずきポイント: `--force` と `--legacy-peer-deps` は「解決」ではない**
...
**[注意] つまずきポイント: optional は「ロックファイルの再現性」の穴になる**
...
この問題は package.json だけでは原理的に解決できません。「範囲」を書いている以上、選択の余地が残るからです。解決策は「実際に選んだ結果」を別のファイルに記録しておくこと——それが [4章](https://npmg.yamauz.workers.dev/basics/04-lockfiles)で扱うロックファイルです。この伏線を覚えたまま読み進めてください。
