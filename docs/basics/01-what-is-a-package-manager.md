...
4 つの仕事は、言葉だけ並べるとどうしても手触りがありません。ここで、本書に何度か顔を出すことになる一軒のラーメン屋に登場してもらいます。屋号は **らぁめん濃度盛汁流(のうどもりじるりゅう)**。字面のとおり `node_modules` のことです。
...
::: warning つまずきポイント: 比喩は入口であって、説明そのものではない
この先も要所でこの店に登場してもらいますが、比喩はあくまで最初のとっかかりです。特に 9 章以降で扱う「ファイルの実体を 1 つだけ持ち、複数の場所から参照する」という仕組みは、丼と麺のたとえでは正確に表せません(1 玉の麺を複数の丼が同時に共有することはできません)。そうした場面では**図書館の書庫**など、別のたとえに乗り換えます。乗り換えるときは、そのつど断ります。
:::
