...
[1章](/basics/01-what-is-a-package-manager)のらぁめん濃度盛汁流でいえば、package.json は厨房に貼られた**品書き**です。そこに書いてあるのは「豚バラ 3 枚」という一点ではなく、「豚バラ 3 枚から 5 枚まで」という**幅**です。`^` は「同じ豚バラである限り、多少の枚数の増減は任せる」、`~` は「増減は 1 枚まで」、記号なしは「きっかり 3 枚」。幅を持たせておけば仕入れの都合に合わせられますが、そのぶん**今日の一杯が何枚だったかは品書きを見てもわからない**。この「わからなさ」が、4 章で仕入れ伝票(ロックファイル)が必要になる理由に直結します。
