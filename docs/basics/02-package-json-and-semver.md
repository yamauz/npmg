...
semver の約束を前提に、package.json では「どこまでの更新を自動で受け入れるか」を記号で指定します。代表は `^`(キャレット)と `~`(チルダ)の 2 つです。
...
このほか `>=1.2.0 <2.0.0` のような不等号や、`1.2.x` のようなワイルドカードも書けますが、実務で目にするのはほぼ `^` です。1 つ注意があり、MAJOR が 0 の間(`0.x.y`)は「まだ何が壊れるかわからない開発版」という扱いで、`^0.2.3` は `0.2.x` の範囲しか許しません。
